...
export class LLMMonitor {
  ...
  startCall(model: string, messages: ChatMessage[], source: string = 'unknown'): string {
    ...
    const inputTokens = this.tokenCalculator.calculateChatTokens(messages, model as SupportedModel).totalTokens;
  ...
  endCall(
    ...
    if (!record) {
      console.warn(`LLM调用记录未找到: ${id}`);
      return;
    }
    
    const responseTime = Date.now() - record.timestamp;
    const outputTokens = response ? this.tokenCalculator.calculateTokens(response, record.model as SupportedModel) : 0;
   ...
   * 清理历史记录
   */
  clearHistory(olderThanHours: number = 168): void { // 默认清理7天前的记录
