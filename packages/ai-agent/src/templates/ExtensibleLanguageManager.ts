...
class SimpleYamlProcessor {
                ...
                if (trimmedLine.includes(':')) {
                    ...
                    if (value) {
                        ...
                        if (value === 'true' || value === 'false') {
                            ...
                            currentObject[key.trim()] = arrayContent.split(',').map(item => item.trim().replace(/"/g, ''));
                        } else {
                        ...
                        stack.push(currentObject[key.trim()]);
    ...
    static dump(obj: any, options?: { indent?: number }): string {
        const indent = options?.indent || 2;
        return SimpleYamlProcessor.objectToYaml(obj, 0, indent);
    }

    private static objectToYaml(obj: any, currentIndent: number, indentSize: number): string {
        ...
        if (typeof obj === 'object') {
            ...
            return entries.map(([key, value]) => {
                const spaces = ' '.repeat(currentIndent);
                const valueStr = SimpleYamlProcessor.objectToYaml(value, currentIndent + indentSize, indentSize);
...
import { LanguageTemplate, AnalysisRule, CodingStandard, SecurityCheck, PerformanceCheck } from './LanguageTemplates';
...
export class ExtensibleLanguageManager {
    ...
    private async loadRegistry(): Promise<void> {
        const registryPaths = [
            ...
            path.join(process.env.HOME || process.env.USERPROFILE || '', '.ai-agent-hub', 'registry.yaml')
        ];
        ...
        for (const registryPath of registryPaths) {
            if (fs.existsSync(registryPath)) {
                try {
                    ...
                    const registry = SimpleYamlProcessor.load(content) as LanguageRegistry;
    ...
    private async loadTemplatesFromDirectory(directory: string): Promise<void> {
        try {
            ...
            console.warn(`Failed to load templates from directory ${directory}:`, error);
        }
    }
    ...
    private async loadTemplateFromFile(filePath: string): Promise<void> {
        try {
            ...
            const template = SimpleYamlProcessor.load(content) as LanguageTemplate;
    ...
    private async loadPluginsFromDirectory(directory: string): Promise<void> {
        try {
            ...
            console.warn(`Failed to load plugins from directory ${directory}:`, error);
        }
    }
     ...
     * 验证模板
     */
    private validateTemplate(template: any): template is LanguageTemplate {
        return template &&
               typeof template.language === 'string' &&
               typeof template.displayName === 'string' &&
               Array.isArray(template.fileExtensions) &&
               Array.isArray(template.analysisRules) &&
               Array.isArray(template.codingStandards) &&
               Array.isArray(template.securityChecks) &&
               Array.isArray(template.performanceChecks);
    }

    /**
     * 验证插件
     */
    private validatePlugin(plugin: any): plugin is LanguagePlugin {
        return plugin &&
               typeof plugin.name === 'string' &&
               typeof plugin.version === 'string' &&
               plugin.template &&
               this.validateTemplate(plugin.template);
    }
            ...
            if (plugin.dependencies) {
                for (const dependency of plugin.dependencies) {
                    if (!this.plugins.has(dependency) && !this.templates.has(dependency)) {
                        console.warn(`Plugin ${pluginId} has unmet dependency: ${dependency}`);
                    }
                }
            }
        }
     ...
     * 获取语言详细信息
     */
    getLanguageInfo(language: string): { template: LanguageTemplate; config: any } | undefined {
    ...
    async saveTemplate(template: LanguageTemplate, directory?: string): Promise<string> {
        ...
        const content = SimpleYamlProcessor.dump(template, { indent: 2 });
    ...
    async saveRegistry(directory?: string): Promise<string> {
        ...
        const content = SimpleYamlProcessor.dump(this.registry, { indent: 2 });
    ...
    getStatistics(): {
        ...
        const enabledPlugins = Array.from(this.plugins.values())
            .filter(plugin => this.registry.plugins[plugin.name]?.enabled !== false).length;
