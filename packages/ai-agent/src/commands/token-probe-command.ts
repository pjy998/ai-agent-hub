...
import { TokenProbe, TokenProbeManager, COPILOT_MODELS, CopilotModel, TokenProbeConfig, TokenProbeResult } from '../features/token-probe';
...
export class TokenProbeCommand {
  ...
  private async showConfigurationDialog(): Promise<TokenProbeConfig | null> {
    ...
    Object.entries(COPILOT_MODELS).forEach(([key, model]) => {
      ...
      const costInfo = model.costMultiplier === 0 
        ? '免费' 
  ...
  private getModelDescription(model: CopilotModel): string {
    const descriptions: Record<CopilotModel, string> = {
      ...
      'gpt-5-mini': 'GPT-5 mini预览版，轻量级模型',
      'gpt-5': 'GPT-5预览版，最先进的语言模型',
      'o3-mini': 'OpenAI o3-mini模型，推理优化',
      'o4-mini': 'OpenAI o4-mini预览版',
      'claude-sonnet-3.5': 'Anthropic Claude Sonnet 3.5模型',
      'claude-sonnet-3.7': 'Anthropic Claude Sonnet 3.7模型',
      'claude-sonnet-4': 'Anthropic Claude Sonnet 4模型',
      'gemini-2.5-pro': 'Google Gemini 2.5 Pro模型',
  ...
  private async showResult(result: TokenProbeResult): Promise<void> {
    ...
    const message = result.status === 'success' 
      ? `${statusIcon} Token Probe完成！${result.model}最大上下文: ${result.maxContextTokens.toLocaleString()} tokens`
