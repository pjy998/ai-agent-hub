...
export class CSharpAnalyzer {
  ...
  private extractClasses(content: string): ClassInfo[] {
    ...
    const classRegex = /^\s*(public|private|protected|internal)?\s*(abstract|static)?\s*class\s+(\w+)(?:\s*:\s*([\w\s,<>]+))?/gm;
    const lines = content.split('\n');
    ...
    while ((match = classRegex.exec(content)) !== null) {
      ...
      // 解析继承关系
      const inheritanceParts = inheritance.split(',').map(s => s.trim()).filter(s => s);
      ...
      const propertyCount = (classContent.match(/\s+(public|private|protected|internal)\s+[\w<>\[\]]+\s+\w+\s*{/g) || []).length;
      const methodCount = (classContent.match(/\s+(public|private|protected|internal)\s+[\w<>\[\]]+\s+\w+\s*\(/g) || []).length;
  ...
  private extractMethods(content: string): MethodInfo[] {
    ...
    const methodRegex = /^\s*(public|private|protected|internal)\s+(static\s+)?(async\s+)?[\w<>\[\]]+\s+(\w+)\s*\(([^)]*)\)/gm;
  ...
  private calculateCodeMetrics(sourceFiles: SourceFileInfo[]): CodeMetrics {
    ...
    const averageMethodComplexity = allMethods.length > 0 
      ? allMethods.reduce((sum, method) => sum + method.cyclomaticComplexity, 0) / allMethods.length 
      : 0;
    const maxMethodComplexity = allMethods.length > 0 
  ...
  private determineProjectType(content: string): 'console' | 'library' | 'web' | 'test' | 'unknown' {
