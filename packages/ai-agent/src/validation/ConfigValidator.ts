            ...
            for (const line of lines) {
                ...
                if (trimmedLine.includes(':')) {
                    ...
                    if (!Array.isArray(currentObject)) {
                        // 如果当前对象不是数组，需要特殊处理
                        console.warn('YAML解析警告：遇到数组项但当前对象不是数组');
                    }
                }
            }
...
export class ConfigValidator {
    ...
    async validateConfig(filePath: string): Promise<{
        ...
        try {
            ...
            const config = SimpleYamlParser.load(content) as any;
    ...
    validateYamlContent(content: string): ValidationResult {
        ...
        try {
            ...
            // 步骤验证
            this.validateSteps(config.steps || [], result);
     ...
     * 测试配置
     */
    async testConfiguration(config: LanguageAnalysisConfig, testCases?: TestCase[]): Promise<TestResult> {
    ...
    private validateSteps(steps: any[], result: ValidationResult): void {
        ...
        steps.forEach((step, index) => {
            ...
            // 检查提示长度
            if (step.prompt && step.prompt.length < 20) {
                result.warnings.push({
                    field: `${stepField}.prompt`,
                    message: '提示过短',
    ...
    private validateOutput(output: any, result: ValidationResult): void {
        if (output.format && !['markdown', 'json', 'html', 'text'].includes(output.format)) {
            result.warnings.push({
                field: 'output.format',
                message: '不支持的输出格式',
    ...
    private validateSecurityChecks(checks: any[], result: ValidationResult): void {
        checks.forEach((check, index) => {
            ...
            if (check.riskLevel && !['high', 'medium', 'low'].includes(check.riskLevel)) {
                result.errors.push({
    ...
    private validatePerformanceChecks(checks: any[], result: ValidationResult): void {
        checks.forEach((check, index) => {
            ...
            if (check.impact && !['high', 'medium', 'low'].includes(check.impact)) {
                result.errors.push({
    ...
    private generateDefaultTestCases(config: LanguageAnalysisConfig): TestCase[] {
        ...
        // 分析规则测试
        config.analysisRules.forEach((rule: any, index: number) => {
            tests.push({
     ...
     * 计算覆盖率
     */
    private calculateCoverage(config: LanguageAnalysisConfig, tests: TestCase[]): TestResult['coverage'] {
        ...
        return {
            ...
            securityChecks: totalSecurityChecks > 0 ? Math.round((securityTests / totalSecurityChecks) * 100) : 100,
            performanceChecks: totalPerformanceChecks > 0 ? Math.round((performanceTests / totalPerformanceChecks) * 100) : 100
        };
    }
     ...
     * 检查循环引用
     */
    private hasCircularReference(obj: any, visited = new Set()): boolean {
