...
export class OptimizedContextCollector {
  ...
  private async processFilesBatch(files: string[]): Promise<void> {
    for (let i = 0; i < files.length; i += this.config.batchSize) {
      ...
      for (const result of batchResults) {
        if (result) {
          this.collectedItems.push(result);
        }
      }
  ...
  private async checkMemoryUsage(): Promise<void> {
    ...
    if (heapUsedMB > 200) { // 200MB threshold
      console.warn(`内存使用过高: ${heapUsedMB.toFixed(2)}MB，触发垃圾回收`);
  ...
  private async collectDirectoryContexts(): Promise<void> {
    ...
    for (const dirPath of directories.slice(0, 50)) { // 限制目录数量
      const item: ContextItem = {
  ...
  private async collectSymbolContexts(): Promise<void> {
    // 从已收集的文件中提取符号，避免重复读取
    const fileItems = this.collectedItems.filter(item => 
      item.type === 'file' && item.content && !item.metadata.skipped
    );
    
    for (const fileItem of fileItems.slice(0, 100)) { // 限制符号提取的文件数量
      if (fileItem.content) {
  ...
  private async collectDependencyContexts(): Promise<void> {
    ...
    try {
      ...
      for (const [name, version] of Object.entries(deps).slice(0, 20)) { // 限制依赖数量
        const item: ContextItem = {
          id: `dependency:${name}`,
          type: 'dependency',
  ...
  private isTextFile(extension: string): boolean {
    const textExtensions = ['.ts', '.js', '.cs', '.json', '.md', '.txt', '.yml', '.yaml', '.xml', '.html', '.css', '.scss'];
  ...
  private extractSymbols(content: string, filePath: string): ContextItem[] {
    ...
    if (ext === '.cs') {
      ...
      if (classMatches) {
        classMatches.slice(0, 10).forEach((match, index) => { // 限制符号数量
