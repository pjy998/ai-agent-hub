...
import { languageManager } from '../templates/ExtensibleLanguageManager';
...
class SimpleYamlGenerator {
  static stringify(obj: Record<string, unknown>, options: { indent?: number } = {}): string {
    const indent = options.indent || 2;
    return this.objectToYaml(obj, 0, indent);
  }

  private static objectToYaml(obj: unknown, depth: number, indent: number): string {
...
export class DynamicConfigGenerator {
   ...
   */
  async generateLanguageConfig(
    language: string,
    requirements: string
  ): Promise<LanguageAnalysisConfig> {
...
`;
    ...
    const yamlConfig = {
      ...
      agents: {
        analyzer: {
          ...
          capabilities: ['代码静态分析', '编码规范检查', '最佳实践建议', '重构建议'],
        },
        reporter: {
          name: 'Report Generator',
          description: '生成分析报告的代理',
          capabilities: ['报告生成', '问题汇总', '改进建议'],
        },
      },
      ...
      output: {
        format: 'markdown',
        sections: ['executive_summary', 'detailed_analysis', 'issues_list', 'improvement_plan'],
  ...
  private generateCodingStandardsPrompt(config: LanguageAnalysisConfig): string {
    const rulesText = config.analysisRules
      .map(rule => `- ${rule.name}: ${rule.description}`)
      .join('\n');

    const standardsText = config.codingStandards
      .map(
        standard =>
  ...
  private generateSecurityPrompt(config: LanguageAnalysisConfig): string {
    const checksText = config.securityChecks
      .map(check => `- ${check.name} (${check.riskLevel}风险): ${check.description}`)
      .join('\n');
  ...
  private generatePerformancePrompt(config: LanguageAnalysisConfig): string {
    const checksText = config.performanceChecks
      .map(check => `- ${check.name} (${check.impact}影响): ${check.description}`)
      .join('\n');
  ...
  getSupportedLanguages(): string[] {
    ...
    // 合并并去重
    const allLanguages = [
      ...new Set([...configLanguages, ...templateLanguages, ...extensibleLanguages]),
    ];
