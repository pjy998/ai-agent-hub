...
/**
 * C# 项目分析 Chat 参与者
 * 在 GitHub Copilot Chat 中提供专门的 C# 项目分析功能
 */
export class CSharpAnalysisParticipant {
  ...
  async handleRequest(
    ...
    try {
      ...
      if (!vscode.workspace.workspaceFolders || vscode.workspace.workspaceFolders.length === 0) {
        stream.markdown('❌ **错误**: 请先打开一个包含 C# 项目的工作区。');
        return;
      }
      ...
      stream.markdown(`❌ **C# 分析错误**: ${errorMessage}`);
      outputManager.logError('C# Chat参与者错误', error instanceof Error ? error : new Error(String(error)));
    }
  }
  ...
  private async handleHelpRequest(stream: vscode.ChatResponseStream): Promise<void> {
    ...
    stream.markdown('🔧 如需更多帮助，请使用VS Code命令面板中的 `AI Agent: Analyze C# Project` 命令。');
  }
  ...
  private async handleDefaultRequest(stream: vscode.ChatResponseStream): Promise<void> {
    stream.markdown('👋 **欢迎使用 C# 项目分析助手！**\n\n');
    
    stream.markdown('我可以帮助您：\n');
    stream.markdown('- 🔍 **分析项目**: `@csharp 分析项目`\n');
    stream.markdown('- 🎯 **质量检查**: `@csharp 质量检查`\n');
    stream.markdown('- 📊 **生成报告**: `@csharp 生成报告`\n');
    stream.markdown('- ⚠️ **显示问题**: `@csharp 显示问题`\n');
    stream.markdown('- 📈 **查看统计**: `@csharp 统计`\n');
    stream.markdown('- ❓ **获取帮助**: `@csharp 帮助`\n\n');
    
    stream.markdown('💡 **提示**: 请确保您的工作区包含有效的 C# 项目文件。');
  }
