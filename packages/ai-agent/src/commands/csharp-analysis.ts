...
import { getCSharpAnalyzer, CSharpProjectInfo, QualityIssue } from '../analyzers/csharp-analyzer';
...
export class CSharpAnalysisCommand {
  ...
  async analyzeCSharpProject(): Promise<void> {
    try {
      ...
      outputManager.logError('C#项目分析失败', error instanceof Error ? error : new Error(String(error)));
    }
  }
  ...
  async analyzeCSharpFolder(uri?: vscode.Uri): Promise<void> {
    try {
      ...
      vscode.window.showErrorMessage(`分析失败: ${error}`);
      outputManager.logError('C#文件夹分析失败', error instanceof Error ? error : new Error(String(error)));
    }
  }
   ...
   * 执行项目分析
   */
  private async performAnalysis(projectPath: string, projectName: string): Promise<CSharpProjectInfo> {
...
` +
           projectInfo.packageReferences.map(pkg => 
...
` +
           projectInfo.projectReferences.map(ref => 
             `- ${ref}`
           ).join('\n') + '\n\n' +
  ...
  private generateIssuesWebview(issues: QualityIssue[], projectName: string): string {
    ...
    return `
      ...
      <body>
        ...
        ${issues.length > 0 ? categorySections : `
          <div class="no-issues">
            <div class="icon">🎉</div>
            <h2>恭喜！没有发现质量问题</h2>
            <p>您的代码质量很好，继续保持！</p>
