...
let tiktoken: unknown = null;
try {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  tiktoken = require('tiktoken');
} catch {
  outputManager.logWarning('tiktoken not available, using fallback token estimation');
}
...
export class TokenCalculator {
  ...
  private encodingCache = new Map<string, unknown>();
  ...
  calculateTokens(text: string, model: SupportedModel = 'gpt-4'): number {
    try {
      ...
      const tokens = (encoding as any).encode(text);
      return tokens.length;
    } catch {
      outputManager.logWarning('Token计算失败，使用估算方法');
      // 回退到估算方法
  ...
  calculateChatTokens(
    ...
    try {
      ...
      for (const message of messages) {
        ...
        const tokens = (encoding as any).encode(messageText);
      ...
      outputManager.logWarning('聊天token计算失败，使用估算方法');
    ...
    if (!this.encodingCache.has(encodingName)) {
      try {
        ...
        if (this.isOpenAIModel(model)) {
          const encoding = (tiktoken as any).encoding_for_model(model);
          this.encodingCache.set(encodingName, encoding);
        } else {
          // 对于非OpenAI模型，使用通用编码器
          const encoding = (tiktoken as any).get_encoding(encodingName);
          this.encodingCache.set(encodingName, encoding);
        }
      } catch {
        // 如果特定编码器不可用，使用cl100k_base作为默认
        const encoding = (tiktoken as any).get_encoding('cl100k_base');
        this.encodingCache.set(encodingName, encoding);
      }
    }
  ...
  clearCache(): void {
    for (const encoding of this.encodingCache.values()) {
      if (encoding && typeof (encoding as any).free === 'function') {
