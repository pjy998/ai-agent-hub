...
export const OPTIMIZATION_PRESETS = {
  ...
  FAST: {
    ...
    skipDirectories: ['node_modules', '.git', 'dist', 'out', 'build', '.vscode', 'bin', 'obj', 'packages'],
    skipExtensions: ['.exe', '.dll', '.so', '.dylib', '.bin', '.obj', '.pdb', '.png', '.jpg', '.gif']
  ...
  LARGE_PROJECT: {
    ...
    skipDirectories: ['node_modules', '.git', 'dist', 'out', 'build', '.vscode', 'bin', 'obj', 'packages', 'vendor'],
    skipExtensions: ['.exe', '.dll', '.so', '.dylib', '.bin', '.obj', '.pdb', '.png', '.jpg', '.gif', '.svg']
  } as OptimizationConfig
};
...
export class OptimizationConfigManager {
  ...
  static loadFromVSCode(): OptimizationConfig {
    ...
    return {
      ...
      enableParallelProcessing: config.get('enableParallelProcessing', baseConfig.enableParallelProcessing),
...
export class PerformanceMonitor {
  ...
  getReport(): {
    ...
    const enhancedCheckpoints = this.checkpoints.map((checkpoint, index) => ({
      ...checkpoint,
      memoryDelta: checkpoint.memory - (index > 0 ? this.checkpoints[index - 1].memory : this.startMemory)
    }));
