...
/**
 * 配置生成器Chat参与者
 * 使用GPT-4.1智能分析用户输入，动态生成YAML分析配置
 */
export class ConfigGeneratorParticipant extends IntelligentParticipant {
  ...
  protected initializeFlows(): void {
    this.flows.set('generate_config', {
      name: '生成配置',
      description: '为指定语言生成YAML分析配置',
      supportedIntents: ['生成', 'generate', '创建', 'create', '配置', 'config'],
      execute: async (request, context, stream, token, analysis) => {
        const intent = this.parseUserIntent(request.prompt);
        await this.executeGenerateConfig(intent, stream, token);
      },
    });

    this.flows.set('list_languages', {
      name: '语言列表',
      description: '显示支持的编程语言列表',
      supportedIntents: ['列表', 'list', '语言', 'language', '支持', 'support'],
      execute: async (request, context, stream, token, analysis) => {
        await this.executeListLanguages(stream);
      },
    });

    this.flows.set('detect_languages', {
      name: '检测语言',
      description: '自动检测项目中的编程语言',
      supportedIntents: ['检测', 'detect', '识别', 'identify', '自动', 'auto'],
      execute: async (request, context, stream, token, analysis) => {
        await this.executeDetectLanguages(stream);
      },
    });

    this.flows.set('validate_config', {
      name: '验证配置',
      description: '验证YAML配置文件的正确性',
      supportedIntents: ['验证', 'validate', '检查', 'check', '校验', 'verify'],
      execute: async (request, context, stream, token, analysis) => {
        const intent = this.parseUserIntent(request.prompt);
        await this.executeValidateConfig(intent, stream, token);
      },
    });

    this.flows.set('test_config', {
      name: '测试配置',
      description: '测试配置文件的功能性',
      supportedIntents: ['测试', 'test', '运行', 'run', '执行', 'execute'],
      execute: async (request, context, stream, token, analysis) => {
        const intent = this.parseUserIntent(request.prompt);
        await this.executeTestConfig(intent, stream, token);
      },
    });

    this.flows.set('manage_plugins', {
      name: '插件管理',
      description: '管理语言插件',
      supportedIntents: ['插件', 'plugin', '管理', 'manage', '扩展', 'extension'],
      execute: async (request, context, stream, token, analysis) => {
        await this.executePlugins(stream);
      },
    });

    this.flows.set('view_stats', {
      name: '统计信息',
      description: '查看配置生成统计信息',
      supportedIntents: ['统计', 'stats', '信息', 'info', '数据', 'data'],
      execute: async (request, context, stream, token, analysis) => {
        await this.executeStats(stream);
      },
    });

    this.flows.set('reload_system', {
      name: '重新加载',
      description: '重新加载配置系统',
      supportedIntents: ['重新加载', 'reload', '刷新', 'refresh', '重启', 'restart'],
      execute: async (request, context, stream, token, analysis) => {
        await this.executeReload(stream);
      },
    });

    this.flows.set('help', {
      name: '帮助信息',
      description: '显示配置生成器使用帮助',
      supportedIntents: ['帮助', 'help', '使用', 'usage', '如何', 'how', '指南', 'guide'],
      execute: async (request, context, stream, token, analysis) => {
        await this.executeHelp(stream);
      },
    });

    this.flows.set('default', {
      name: '默认响应',
      description: '提供默认的配置生成器介绍',
      execute: async (request, context, stream, token, analysis) => {
        await this.executeUnknownIntent(request.prompt, stream);
      },
    });
  ...
  private parseUserIntent(prompt: string): {
    ...
    // 列出支持的语言
    if (
      lowerPrompt.includes('支持') ||
      lowerPrompt.includes('语言') ||
      lowerPrompt.includes('list') ||
      lowerPrompt.includes('languages')
    ) {
      return { action: 'list' };
    }
    ...
    // 检测项目语言
    if (
      lowerPrompt.includes('检测') ||
      lowerPrompt.includes('识别') ||
    ...
    // 帮助信息
    if (
      lowerPrompt.includes('帮助') ||
      lowerPrompt.includes('help') ||
      lowerPrompt.includes('使用') ||
      lowerPrompt.includes('怎么')
    ) {
      return { action: 'help' };
    }
   ...
   * 执行生成配置流程
   */
  private async executeGenerateConfig(
    ...
    if (existingConfig) {
      stream.markdown(
        `✅ 发现已有的 ${intent.language.toUpperCase()} 配置，正在生成YAML文件...\n\n`
      );
      ...
      try {
        ...
        stream.markdown(
          `❌ **生成YAML文件失败**: ${error instanceof Error ? error.message : '未知错误'}`
        );
      }
    } else {
      // 需要通过AI生成新配置
      ...
      try {
        ...
        const newConfig = await this.configGenerator.generateLanguageConfig(
          intent.language,
          intent.requirements || ''
        );
        ...
        stream.markdown(
          `❌ **AI配置生成失败**: ${error instanceof Error ? error.message : '未知错误'}\n\n`
        );
   ...
   * 执行列出支持语言流程
   */
  private async executeListLanguages(stream: vscode.ChatResponseStream): Promise<void> {
   ...
   * 执行检测项目语言流程
   */
  private async executeDetectLanguages(stream: vscode.ChatResponseStream): Promise<void> {
    ...
    try {
      const detectedLanguages = await this.configGenerator.detectProjectLanguages();
   ...
   * 执行帮助信息流程
   */
  private async executeHelp(stream: vscode.ChatResponseStream): Promise<void> {
   ...
   * 执行插件管理流程
   */
  private async executePlugins(stream: vscode.ChatResponseStream): Promise<void> {
   ...
   * 执行统计信息流程
   */
  private async executeStats(stream: vscode.ChatResponseStream): Promise<void> {
   ...
   * 执行重新加载流程
   */
  private async executeReload(stream: vscode.ChatResponseStream): Promise<void> {
    ...
    try {
      ...
      stream.markdown(
        `- 插件语言: ${plugins.flatMap((p: LanguagePlugin) => p.supportedLanguages).length} 种\n\n`
      );
   ...
   * 执行未知意图流程
   */
  private async executeUnknownIntent(
    prompt: string,
    stream: vscode.ChatResponseStream
  ): Promise<void> {
   ...
   * 执行验证配置流程
   */
  private async executeValidateConfig(
    ...
    try {
      ...
      if (result.isValid) {
        ...
        stream.markdown(`- 步骤数量: ${(result as any).config?.steps?.length || 0}\n`);
   ...
   * 执行测试配置流程
   */
  private async executeTestConfig(
    intent: { filePath?: string },
    stream: vscode.ChatResponseStream,
    token: vscode.CancellationToken
  ): Promise<void> {
    ...
    try {
      ...
      if (result.stepResults.length > 0) {
        ...
        result.stepResults.forEach((stepResult, index) => {
          ...
          if (stepResult.output) {
            stream.markdown(
              `   输出: ${stepResult.output.substring(0, 100)}${stepResult.output.length > 100 ? '...' : ''}\n`
            );
          }
      ...
      stream.markdown(
        `❌ **测试过程出错**: ${error instanceof Error ? error.message : '未知错误'}`
      );
    }
  }
